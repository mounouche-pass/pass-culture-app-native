...
describe('getRecommendationParameters', () => {
  ...
  it('should return parameters with mapped categories when parameters are provided (isFree, isEvent ans isDuo are true)', () => {
  ...
  it('should return parameters with mapped categories when parameters are provided (isFree, isEvent ans isDuo are false)', () => {
    const parameters: RecommendationParametersFields = {
      title: 'some parameters',
      categories: ['Arts & loisirs créatifs', 'Bibliothèques, Médiathèques', 'Cartes jeunes'],
      isFree: false,
